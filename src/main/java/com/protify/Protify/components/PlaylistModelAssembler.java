    ...
    public EntityModel<Playlist> toModel(Playlist entity) {
        HalModelBuilder builder = HalModelBuilder.halModelOf(entity)
                .link(links.linkToItemResource(entity, Playlist::getId)
                        .andAffordance(afford(methodOn(PlaylistController.class).deletePlaylist(entity.getId())))
                        .andAffordance(afford(methodOn(PlaylistController.class).patchPlaylist(entity.getId(), new PlaylistTitleDto())))
                )
